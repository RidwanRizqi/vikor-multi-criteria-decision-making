            ...
            <form action="{{ route('values.store') }}" method="POST">
                            ...
                            <input type="number" onchange="setTwoNumberDecimal" min="0" step="0.01"
                                   @php
                                        $penilaianForCriteria = $value
                                             ...
                                             ->first();
                                      @endphp
                                value="{{ $penilaianForCriteria ? $penilaianForCriteria->score : 0 }}"
                ...
                <div
                    ...
                    <button type="submit" data-toggle="modal" data-target="#import"
                        class="inline-block px-8 py-2 m-1 mb-4 text-xs font-bold text-center text-white uppercase align-middle transition-all border-0 rounded-lg cursor-pointer ease-soft-in leading-pro tracking-tight-soft bg-gradient-to-tl from-blue-700 to-blue-200 shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85">Upload</button>
                </div>
            </form>
        </div>
    </div>
